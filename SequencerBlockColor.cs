 ...
 */
public class SequencerBlockColor : MonoBehaviour
{
    ...
    void Update()
    {
         ...
         * Gets bool stored in array from Sequence808 script
         * When cube is inactive (false) - defaultColor
         * When cube is active (true) - activeColor
         */
        if (Sequence808.pattern[patternIndex] == false)
        {
            var colorRender = gameObject.GetComponent<Renderer>();
            colorRender.material.SetColor("_Color", defaultColor);
        }
        else if (Sequence808.pattern[patternIndex] == true)
        {
            var colorRender = gameObject.GetComponent<Renderer>();
